...
function initialRoutineBegin(snapshot) {
  return async function () {
    ...
    initialComponents.forEach( function(thisComponent) {
...
function instrustionRoutineBegin(snapshot) {
  return async function () {
    ...
    instrustionComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function distributeRoutineBegin(snapshot) {
  return async function () {
    ...
    distributeComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function blank1RoutineBegin(snapshot) {
  return async function () {
    ...
    blank1Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
